...
## 1-2 MCP.resource 실습(tutorial_2.py)
...
## 1-3 MCP.prompt 실습(tutorial_3.py)
...
```
download_organizer 도구를 이용해서 다운로드 폴더를 정리해줘
```
...
- 도구 목록에서 '카카오톡 나쳇방'을 찾아서 상세페이지에 접속
- [도구함에 추가] 버튼 클릭하여 추가
- 클로드 앱의 [커넥터 둘러보기]에서 'PlayMCP'를 검색해서 [+] 버튼 클릭
- 인증창이 나오면 [모두 동의 후 계속하기]를 눌러 권한을 허용
- 클로드 데스크탑 앱에서 입력창의 [+] 버튼을 눌러 PlayMCP 활성화
```
회의록 결과 보고 예제를 작성해 주고 요약한 내용을 카카오톡 나쳇방으로 보내줘.    
```
...
## 9) 다단계 파이프라인 구성: 시장조사에서 발표자료까지

###  9-1 단계별 자료구성

- 1단계: 웹 검색 : 노화 치료제 개발 현황 정보를 웹에서 수집하여 엑셀로 저장
- 2단계: 엑셀: 비교표와 차트 추가
- 3단계: 워드: 요약 보고서 생성
- 4단계: 임원 발표용 슬라이드 생성
...
- 1단계:
```
웹에서 노화관련 치료약 개발 현황을 치료 방식, 주요 개발 주체, 상용화 예상시기, 예상 가격  등을 조사해서 정리해줘. 결과를 age_overcome.xlsx로 저장해줘
```

- 2단계:
```
age overcome.xlsx에 비교 차트가 포함된 분석 시트를 추가해 줘
```
- 3단계: 무료 버전에서는 2단계 이상 진행이 어려움(4시간 이후 가능)
```
분석내용을 '노화연구분석보고서.docx'로 정리 해줘. 형식: 요약(1페이지) + 상세 비교표 + 시사점 3가지
```

- 4단계:
```
노화연구분석보고서.docx 내용을 참고해서 임원 발표용 5장 ppt로 만들어줘. 파일명은 '노화연구분석_발표.pptx'로 저장해줘.
```
